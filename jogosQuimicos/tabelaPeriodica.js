...
const toggleNomes = document.getElementById('toggle-nomes');
...
function obterElementosPorModo(modo) {
    switch (modo) {
        case 'grupos':
            // Modo fácil: apenas os primeiros elementos de cada grupo
            const primeirosElementosGrupos = [
                1,   // Hidrogênio (Grupo 1)
                4,   // Berílio (Grupo 2)
                21,  // Escândio (Grupo 3)
                22,  // Titânio (Grupo 4)
                23,  // Vanádio (Grupo 5)
                ...
                5,   // Boro (Grupo 13)
                6,   // Carbono (Grupo 14)
                7,   // Nitrogênio (Grupo 15)
                8,   // Oxigênio (Grupo 16)
                9,   // Flúor (Grupo 17)
                2    // Hélio (Grupo 18)
            ];
            return elementosQuimicos.filter(el => primeirosElementosGrupos.includes(el.numero));
        case 'completo':
        default:
    ...
    for (let periodo = 1; periodo <= 7; periodo++) {
        for (let grupo = 1; grupo <= 18; grupo++) {
            ...
            if (elemento) {
                ...
                // Elementos na tabela inicialmente vazios - informações aparecem apenas quando posicionados corretamente
...
function permitirDrop(e) {
    ...
    alvo.classList.add('drop-target');
}

function removerHover(e) {
    const alvo = e.currentTarget;
    alvo.classList.remove('drop-target');
}

function soltarElemento(e) {
    e.preventDefault();
    const elementoAlvo = e.currentTarget;
    elementoAlvo.classList.remove('drop-target');

    const dataTransferText = e.dataTransfer.getData('text/plain');
    if (!dataTransferText) return;
    const elementoData = JSON.parse(dataTransferText);

    // Validar alvo
    if (!elementoAlvo.classList.contains('elemento')) return;
    // Impedir sobrescrita de posição já correta
    if (elementoAlvo.classList.contains('correto')) return;
...
function posicionarElementoCorreto(alvo, elemento) {
    ...
    alvo.dataset.grupo = elemento.grupo;

    // Atualizar conteúdo visual
    atualizarConteudoElemento(alvo, elemento);

    jogoAtual.elementosPosicionados.add(parseInt(elemento.numero));
    ...
    // Se for o primeiro elemento de um grupo no modo "grupos", completar todo o grupo
    if (jogoAtual.modo === 'grupos') {
        completarGrupoAutomaticamente(elemento.grupo, elemento.numero);
    }
}

// Função para completar automaticamente todo o grupo quando o primeiro elemento é posicionado
function completarGrupoAutomaticamente(grupo, numeroPrimeiroElemento) {
    // Garantir que estamos trabalhando com numbers para comparação
    const grupoNum = parseInt(grupo);
    const numeroElementoNum = parseInt(numeroPrimeiroElemento);

    // No modo "grupos", todos os elementos disponíveis são primeiros elementos dos seus grupos
    // Então sempre completamos o grupo quando um elemento é posicionado corretamente

    // Encontrar todos os outros elementos do mesmo grupo
    const outrosElementosGrupo = elementosQuimicos.filter(el =>
        el.grupo === grupoNum && el.numero !== numeroElementoNum
    );

    // Posicionar cada elemento do grupo automaticamente
    outrosElementosGrupo.forEach(elemento => {
        // Encontrar a célula da tabela para este elemento
        const celulaTabela = document.querySelector(
            `.elemento[data-periodo="${elemento.periodo}"][data-grupo="${elemento.grupo}"]`
        );

        if (celulaTabela && !celulaTabela.classList.contains('correto')) {
            // Posicionar o elemento automaticamente
            celulaTabela.classList.remove('vazio');
            celulaTabela.classList.add('correto');
            celulaTabela.dataset.numero = elemento.numero;
            celulaTabela.dataset.simbolo = elemento.simbolo;
            celulaTabela.dataset.nome = elemento.nome;
            celulaTabela.dataset.grupo = elemento.grupo;

            // Atualizar conteúdo visual
            atualizarConteudoElemento(celulaTabela, elemento);

            // Adicionar aos elementos posicionados (mas NÃO incrementar elementosCorretos)
            jogoAtual.elementosPosicionados.add(parseInt(elemento.numero));
        }
    });

    // Atualizar o progresso após completar o grupo
    atualizarProgresso();

    // Mensagem de feedback (apenas se o jogo não foi completado)
    if (!jogoAtual.jogoCompleto) {
        statusTexto.textContent = `Grupo ${grupo} completo! Todos os elementos foram posicionados automaticamente.`;
        setTimeout(() => {
            if (!jogoAtual.jogoCompleto) {
...
function atualizarProgresso() {
    ...
    if (jogoAtual.modo === 'grupos') {
        // No modo grupos, progresso é baseado apenas nos primeiros elementos posicionados manualmente
        const primeirosElementosPosicionados = jogoAtual.elementosSelecionados.filter(el =>
            jogoAtual.elementosPosicionados.has(parseInt(el.numero))
        ).length;
        const totalPrimeirosElementos = jogoAtual.elementosSelecionados.length;
        progresso = (primeirosElementosPosicionados / totalPrimeirosElementos) * 100;
    } else {
        // No modo completo, usa a lógica normal
        const totalElementos = jogoAtual.elementosSelecionados.length;
        progresso = (jogoAtual.elementosCorretos / totalElementos) * 100;
    }
    ...
    if (!jogoAtual.jogoCompleto) {
        if (jogoAtual.modo === 'grupos') {
            // No modo grupos, jogo completo quando todos os primeiros elementos foram posicionados
            const primeirosElementosPosicionados = jogoAtual.elementosSelecionados.filter(el =>
                jogoAtual.elementosPosicionados.has(parseInt(el.numero))
            ).length;
            if (primeirosElementosPosicionados === jogoAtual.elementosSelecionados.length) {
                jogoAtual.jogoCompleto = true;
                completarJogo();
            }
        } else {
            // No modo completo, usa a lógica normal
            if (jogoAtual.elementosCorretos === jogoAtual.elementosSelecionados.length) {
                jogoAtual.jogoCompleto = true;
                completarJogo();
            }
        }
...
function mostrarTooltip(e) {
    ...
    if (elemento.classList.contains('elemento') && elemento.classList.contains('correto') && elemento.dataset.nome) {
...
toggleNomes.addEventListener('change', () => {
    document.querySelectorAll('.elemento').forEach(el => {
        const nomeDiv = el.querySelector('.elemento-nome');
        if (toggleNomes.checked) {
            if (!nomeDiv) {
                const nome = document.createElement('div');
                nome.className = 'elemento-nome';
                nome.textContent = el.dataset.nome;
                el.appendChild(nome);
            }
        } else {
